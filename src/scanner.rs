...
pub struct RegLastDef([u32; RegId::COUNT]);
...
fn push_unique<A: smallvec::Array<Item = u32>>(deps: &mut SmallVec<A>, val: u32) {
...
pub fn scan_pass1_bytes(
    ...
    if profile {
        ...
        eprintln!("[profile] 已解析行数       : {}", parsed_count);
...
/// - 其它: `elem_width`
fn mem_access_width(class: InsnClass, elem_width: u8, line: &crate::types::ParsedLine) -> u8 {
    match class {
        ...
        InsnClass::SimdLoad | InsnClass::SimdStore => {
            ...
            let data_reg_count = line.base_reg.map_or(1u8, |base| {
                line.operands
                    .iter()
                    .take_while(|op| op.as_reg() != Some(base))
                    .filter(|op| op.as_reg().is_some())
                    .count() as u8
            });
